...
@Client.on_message(command(["الاوامر","reload", f"reload@{BOT_USERNAME}"]) & other_filters)
...
async def update_admin(client, message):
    ...
    await message.reply_text(
        "✅تم إعادة تحميل البوت ** بشكل صحيح! **  \n✅ ** تم تحديث قائمة المسؤولين ↔️الاوامر 1 /تشغيل +اسم الاغنية ↔️2 /تشغيل_فيديو +اسم الفيديو↔️3 /التالي لخطي الاغنية ↔️4 /ايقاف لايقاف التشغيل ↔️5 اوامر التحمل وهي (/اغنيه)+ اسمها لتحميل ملف صوتي و (/فيديو) لتحمل ملف فيديو ↔️لجلب مساعد البوت أكتب امر (/المساعد) ** **! ** "
    )


@Client.on_message(command(["التالي","skip", f"skip@{BOT_USERNAME}", "vskip"]) & other_filters)
@authorized_users_only
...
@Client.on_message(
    command(["ايقاف","stop", f"stop@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["موقت","pause", f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["اكتم","mute", f"mute@{BOT_USERNAME}", "vmute"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["الصوت","volume", f"volume@{BOT_USERNAME}", "vol"]) & other_filters
)
@authorized_users_only

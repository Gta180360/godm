...
@Client.on_message(command(["اغنيه","song", f"song@{bn}"]) & ~filters.edited)
def song(_, message):
...
@Client.on_message(
    command(["فيديو","vsong", f"vsong@{bn}", "video", f"video@{bn}"]) & ~filters.edited
)
...
@Client.on_message(command(["ابحث","lyric", f"lyric@{bn}"]))
